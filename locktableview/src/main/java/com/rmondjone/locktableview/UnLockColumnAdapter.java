...
public class UnLockColumnAdapter extends RecyclerView.Adapter<UnLockColumnAdapter.UnLockViewHolder> {
    ...
    public void onBindViewHolder(UnLockViewHolder holder, final int position) {
        ...
        if (isLockFristRow) {
           if (position == 0){
                createRowView(holder.mLinearLayout, datas, false, mRowMaxHeights.get(position + 1),true);
            } else{
               //第一行是锁定的
               createRowView(holder.mLinearLayout, datas, false, mRowMaxHeights.get(position + 1),false);
           }
        } else {
            if (position == 0) {
                holder.mLinearLayout.setBackgroundColor(ContextCompat.getColor(mContext, mFristRowBackGroudColor));
                createRowView(holder.mLinearLayout, datas, true, mRowMaxHeights.get(position),false);
            } else if (position == 1){
                createRowView(holder.mLinearLayout, datas, false, mRowMaxHeights.get(position),true);
            } else{
                createRowView(holder.mLinearLayout, datas, false, mRowMaxHeights.get(position),false);
            }
        }
        //添加事件
     ...
     */
    private void createRowView(LinearLayout linearLayout, List<String> datas, boolean isFristRow, int mMaxHeight,boolean isSecondRow) {
        //设置LinearLayout
        ...
        for (int i = 0; i < datas.size(); i++) {
            ...
            if (isFristRow) {
                ...
                //数据第一行背景色改变
                linearLayout.setBackgroundColor(ContextCompat.getColor(mContext, R.color.color_eff4f4));
            } else {
                //TODO tupperware自定义，如果为负数，则红色显示
                if (!TextUtils.isEmpty(datas.get(i)) && datas.get(i).startsWith("-")){
