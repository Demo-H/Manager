...
public class PageBehavior  extends CoordinatorLayout.Behavior {
    ...
    public PageBehavior(Context context, AttributeSet attrs) {
        ...
        handler = new Handler();
        gap = context.getResources().getDimensionPixelSize(R.dimen.dimen_100dp);
    }
    ...
    public void onNestedScroll(CoordinatorLayout coordinatorLayout, View child, View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        ...
        if (status == PAGE_ONE) {
            ...
            if (dependentView.get().getTranslationY() < 0 && dyConsumed < 0) {
                mode = 1;
                ((PageNestedScrollView) dependentView.get()).setScrollAble(false, dependentView.get().getTranslationY());
            }
            ...
            if (dependentView.get().getTranslationY() > -dependentView.get().getHeight() && dyConsumed > 0) {
                mode = 1;
                ((PageNestedScrollView) child).setScrollAble(false, dependentView.get().getTranslationY());
            }
        }
    }
